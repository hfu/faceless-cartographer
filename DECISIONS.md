...
**2026-07-21 追記2(ADR提案・複数Example切り替えの実装)**: 上記2つのフォローアップに対応した。

- `UNopenGIS/staccato-spec` へ ADR 0007(`required_styles`/`optional_styles`)を提案するPRを提出([UNopenGIS/staccato-spec#4](https://github.com/UNopenGIS/staccato-spec/pull/4))。
- `EXAMPLE_MAP_INTENT` の切り替えは、単純な置き換えではなく **D40**(下記)として実装した。

## D40: フォーム初期値を複数の Example Map Intent からドロップダウンで選択可能にする

**Status**: Accepted

**Context**: D39 で `required_styles` を実装した後、「フォームの初期表示を石狩川治水の例から火山土地条件図の例に切り替えるべきか」が課題として残っていた。単純な置き換えは2つの理由で却下した。

1. **石狩川治水の例には既に実績がある**(D24 の指標駆動ループで検証済み)ため、削除するのは惜しい。
2. **火山土地条件図の例を新たに初期値にする前に、「Prompt → Ask → Paste」のループが実際に機能するか(既存の Staff プロンプトから該当する自然言語の問いを与えて `required_styles` を含む Map Intent を実際に生成できるか)を検証する必要があった。** 検証の結果、`hfu/layers-martin` の `STAFF_PROMPT.md` は `required_styles` を一切教えておらず(`vlcm`/`vbm` は `required_layers` の `source_id` としてのみ言及)、現状のプロンプトのままでは「火山土地条件図を見たい」という問いに対して `required_styles` ベースの Map Intent を生成できないことが判明した。

**Decision**:

1. **`hfu/layers-martin` の `STAFF_PROMPT.md` を更新**し、`required_styles`/`optional_styles`(D39)を教える節・使い分けの目安・動作確認済みの例3(「北海道の火山土地条件図を見たい」)を追加した。
2. **`scripts/eval-intent.ts`(評価ハーネス)を拡張**し、`resolveStyles` も呼び出して M2(解決率)/M3(描画可能率)にスタイルの解決結果を合算するようにした(従来は `resolveLayers` のみ)。
3. **プロンプト更新の効果を経験的に検証した**: 更新後のプロンプト全文のみを与えた独立エージェントに、プロンプト自身の動作確認済み例3とは異なる具体例(「恵山の火山土地条件図が見たい」)で Staff を演じさせ、`required_styles: [{style_id: "vlcm"}]` を正しく導けることを確認(単なる例3のコピペではなく、一般的な使い分け指針からの汎化であることを担保するため、あえて異なる火山を選んだ)。生成された Map Intent はハーネスで M1〜M5 すべて合格。この結果を `scripts/example-intents/07-volcano-land-condition-map.yaml`(プロンプトの例3と同一)・`08-volcano-land-condition-map-esan.yaml`(恵山、汎化の証拠)としてフィクスチャ化した(スイート8/8合格)。
4. **フォームに `<select id="example-select">` を追加**し、複数の Example Map Intent(現在2件: 石狩川治水[レイヤー構成]・北海道の火山土地条件図[D39のスタイル参照])を切り替え可能にした。切り替えると「2. Ask your AI」のサンプル質問(EN/JA)と「3. Paste」の Map Intent が連動して更新される(ペアがずれないよう、質問とYAMLを1つのオブジェクトにまとめて管理する)。

**Consequences**:
- `src/render.ts`: `EXAMPLE_MAP_INTENT`(単一定数)を `EXAMPLES: MapIntentExample[]`(配列)に置き換え、`renderFormView` にドロップダウンと連動ロジックを追加。
- 石狩川治水の例は保持されたまま、火山土地条件図の例が追加された(どちらも「ワンクリックで動作確認できる」実績を持つ)。
- `scripts/eval-intent.ts`・`scripts/example-intents/`(07・08追加、README更新)。
- ブラウザで両方の例の切り替え・描画・パネルの表示/非表示トグルを確認済み(コンソールエラー無し)。
