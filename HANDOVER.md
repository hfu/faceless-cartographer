...
- **2026-07-21: Map Intent に `required_styles`/`optional_styles` を追加(D39、Issue #6、実装完了)**
  ...
  - **次の担当者へのフォローアップ(未着手)**:
    - `stars.optgeo.org` の Martin サーバー設定に `hfu/kitavolca` のスタイルを `styles.vlcm` として実際に公開する作業(本リポジトリ外のインフラ作業)。公開され次第、`EXAMPLE_MAP_INTENT`(`src/render.ts`)を `required_styles` を使う例に更新するのが自然な次の一歩。
