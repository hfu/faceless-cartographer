...
## 現在の状態(2026-07-09 時点、Phase 1-2 完了)

- **2026-07-09: maplibre-gl-layer-control によるレイヤーパネル統合(D30、試験的実装完了)**
  - ユーザーリクエストに対応: layer visibility/grouping のコントロール可能化
  - UI 層数制御: 25-30 internal layers → 4-10 UI items に自動圧縮（sub-layer 非表示化）
  - グループ化: 「主題レイヤー」「補助情報」で視認性確保
  - Placement: 右下に配置、凡例パネルの上
  - Status: typecheck/build/test OK。本番環境でのブラウザ動作確認は次フェーズ
