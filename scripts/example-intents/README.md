...
All six should report `RESULT: PASS` (suite 6/6). A failure means either a
catalog `source_id` was renamed/removed upstream, or a layer lost data
coverage over its `area.bbox` — both worth investigating.
...
| 06-volcano-geology | 北海道（道央）の火山地質図 | second catalog (stars.optgeo, vector) |
